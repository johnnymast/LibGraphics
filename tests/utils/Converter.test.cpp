...
//
//     CHECK(pixel[0] == 0);   // Blue channel
//     CHECK(pixel[1] == 0);   // Green channel
//     CHECK(pixel[2] == 255); // Red channel
// }
//
...
//
//     CHECK(img.data[centerIdx + 0] > 200); // Red
//     CHECK(img.data[centerIdx + 1] < 50);  // Green
//     CHECK(img.data[centerIdx + 2] < 50);  // Blue
// }
//
...
//     // Create an RGB Mat
//     cv::Mat rgbMat(15, 15, CV_8UC3, cv::Scalar(150, 100, 50)); // BGR input
//
...
//
//     // Check pixel values
//     CHECK(img.data[0] == 50);  // Red
//     CHECK(img.data[1] == 100); // Green
//     CHECK(img.data[2] == 150); // Blue
// }
//
// TEST_CASE("Converter::MatToImage - RGBA", "[Converter]") {
//     // Create an RGBA Mat
//     cv::Mat rgbaMat(10, 10, CV_8UC4, cv::Scalar(90, 60, 30, 255));
...
//
//     // Check pixel values
//     CHECK(img.data[0] == 30); // R
//     CHECK(img.data[1] == 60); // G
//     CHECK(img.data[2] == 90); // B
//     CHECK(img.data[3] == 255); // A
// }
//
...
TEST_CASE("Converter::pixToImage - Grayscale", "[Converter]") {
    Pix* grayPix = pixCreate(10, 10, 8);
    pixSetAllArbitrary(grayPix, 200); // fill with gray value 200

    Image img = Converter::pixToImage(grayPix);

    CHECK(img.width == 10);
    CHECK(img.height == 10);
    CHECK(img.channels == 1);
    CHECK(img.data.size() == 100);
    CHECK(img.data[0] == 200);

    pixDestroy(&grayPix);
}

TEST_CASE("Converter::pixToImage - Empty Pix", "[Converter]") {
    Pix* nullPix = nullptr;
    CHECK_THROWS_AS(Converter::pixToImage(nullPix), std::runtime_error);
}

TEST_CASE("Converter::pixToImage - Unsupported Depth", "[Converter]") {
    Pix* pix1bpp = pixCreate(10, 10, 1); // 1-bit image
    CHECK_THROWS_AS(Converter::pixToImage(pix1bpp), std::runtime_error);
    pixDestroy(&pix1bpp);
}


TEST_CASE("Converter Round-Trip - Image to Pix to Image", "[Converter]") {
    Image original;
    original.width = 4;
    original.height = 4;
    original.channels = 3;
    original.data.resize(4 * 4 * 3, 0);

    // Set pixel at (0,0) to R=255, G=100, B=50
    original.data[0] = 255; // R
    original.data[1] = 100; // G
    original.data[2] = 50;  // B

    Pix* pix = Converter::imageToPix(original);
    Image roundTrip = Converter::pixToImage(pix);

    CHECK(roundTrip.width == original.width);
    CHECK(roundTrip.height == original.height);
    CHECK(roundTrip.channels == 4);

    REQUIRE(roundTrip.data[0] == 255); // R
    REQUIRE(roundTrip.data[1] == 255); // G
    REQUIRE(roundTrip.data[2] == 100);  // B
    REQUIRE(roundTrip.data[3] == 50); // A
